...
export const Packing = (props: IPackingProps) => {
        ...
        >
          {packingList && packingList.length > 0 ? (
            <Table responsive>
              ...
              <tbody>
                {packingList.map((packing, i) => (
                  <tr key={`entity-${i}`}>
                    <td>
                      <Button tag={Link} to={`${match.url}/${packing.id}`} color="link" size="sm">
                        {packing.id}
                      </Button>
                    </td>
                    <td>{packing.name}</td>
            ...
            !loading && <div className="alert alert-warning">No Packings found</div>
          )}
        </InfiniteScroll>
      </div>
    </div>
