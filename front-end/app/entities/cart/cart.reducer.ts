...
export const ACTION_TYPES = {
  SEARCH_CARTS: 'cart/SEARCH_CARTS',
...
export default (state: CartState = initialState, action): CartState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_CARTS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_CARTS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_CARTS):
...
const apiSearchUrl = 'api/_search/carts';

// Actions

export const getSearchEntities: ICrudSearchAction<ICart> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_CARTS,
  payload: axios.get<ICart>(`${apiSearchUrl}?query=${query}`)
});
